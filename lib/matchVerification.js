...
export function verifyMatch(requested, found) {
  ...
  const askedArtists = splitArtists(requested?.artist);
  const primaryAsked = askedArtists[0] ?? requested?.artist;
  const artistOk = foundArtists.some(
    (candidate) => similarity(primaryAsked, candidate) >= ARTIST_SIMILARITY_THRESHOLD,
  );
  if (!artistOk) return { ok: false, reason: 'artist_mismatch' };
