...
export function normalizeName(value) {
  ...
  text = text
    // Strip a trailing "feat. X" credit. Two guards, both learned the hard way:
    //   - the lookbehind stops a title that BEGINS with a credit word from
    //     being erased to nothing;
    //   - "with" is deliberately absent. It is far more often a real word in a
    //     title than a credit marker, and Spotify writes collaborations as
    //     "(with X)", which the bracket-stripping above already removes.
    // Without these, "With or Without You" normalised to "" and U2 could
    // never be matched against itself.
    .replace(/(?<=.)\b(?:feat|ft|featuring)\b[\s.].*$/, ' ')
    .replace(/&/g, ' and ')
