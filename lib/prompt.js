...
export function buildScorePrompt({ input, duration, discovery }) {
  const quota = quotaFor(duration);
  return `A visitor described a scent. Compose their Drydown Score.
...
DO NOT PROPOSE THESE TRACKS AGAIN — they could not be found:
${tracks}
...
repeated, one sensory sentence per track.`;
}
