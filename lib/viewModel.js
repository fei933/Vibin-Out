...
export function toScoreViewModel(doc) {
  ...
  return {
    ...
    // Why it is short, so the page can say something true. `runtimeShort` means
    // the records resolved but do not add up to the runtime the pill promised;
    // `partial` means the provider stopped answering mid-lookup.
