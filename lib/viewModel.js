...
import { spotifySearchUrl, tracklistText } from './spotifyExport.js';
...
export function toScoreViewModel(doc) {
  ...
  const phases = (result.phases ?? []).map((phase) => ({
    ...
    tracks: (phase.tracks ?? []).map((track) => ({
      ...
      // Keyless "open in Spotify" — the export's tier-2 fallback, and the
      // whole playback surface if the client-credentials key ever dies.
      searchUrl: spotifySearchUrl(track.title, track.artist),
  ...
  /**
   * The export's flat view of the same tracks, in playing order across the
   * phases — the order they go into a Spotify playlist, and the order the
   * tier-2 list reads. Handlebars cannot flatten, so it is computed here like
   * every other derived value.
   *
   * NOT deduped: two phases may legitimately draw the same record, and a
   * playlist that silently loses a track disagrees with the page above it.
   */
  const exportTracks = phases.flatMap((phase) =>
    phase.tracks.map(({ title, artist, spotifyId, searchUrl }) => ({
      title,
      artist,
      spotifyId,
      searchUrl,
  ...
  return {
    ...
    artwork,
    exportTracks,
    // Rendered into a textarea, so "copy the list" still works when the
    // clipboard API is unavailable or refused: select-and-copy is the floor.
    tracklist: tracklistText(exportTracks),
