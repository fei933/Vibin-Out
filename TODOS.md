...
- **Photo input** — "upload a picture of your space" as a second input mode. Same pipeline: a vision-capable LLM call produces the same zod brief schema; everything downstream (TrackResolver, score page) is unchanged. Crowded market (PhotoVibe, Picture to Playlist) so it's an *addition*, not the identity — scent stays the brand.
